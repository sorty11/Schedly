  ...
  static Future<bool> elevateToFaculty({
    ...
    if (legacySnap.exists) {
      ...
      if (querySnap.docs.isNotEmpty) {
        if (querySnap.docs.length > 1) {
          throw AppException(
            'Multiple profiles found for this name. Please contact admin.',
          );
        }
        ...
        if (user == null) {
          throw AppException(
            'You must be logged in to create a faculty profile',
          );
        }
        uidToUse = user.uid;
        isNew = true;
    ...
    try {
      ...
      if (uidToUse != user.uid && uidToUse.startsWith('fac_')) {
        ...
        batch.delete(
          FirebaseFirestore.instance.collection('users').doc(legacyId),
        );
    ...
    final assignedDivisions = List<String>.from(
      profileData['assignedDivisions'] ?? [],
    );
