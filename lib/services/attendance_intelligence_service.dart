...
class AttendanceIntelligenceService {
  ...
  static SubjectIntelligence evaluateSubject(AttendanceRecord record) {
    ...
    final safeMargin = _calculateClassesUntil(record.present, total, thresholdWarning, false);
    final classesUntilCritical = _calculateClassesUntil(record.present, total, thresholdCritical, false);
    final recoveryClassesNeeded = _calculateClassesUntil(record.present, total, thresholdCritical, true);

    String? insight;
    if (total > 0) {
      if (pct >= 0.95) insight = 'You can comfortably miss several classes.';
      else if (pct >= 0.85) insight = 'No immediate action required.';
      else if (pct >= 0.80) insight = 'Keep attending regularly.';
      else if (pct >= 0.75) insight = 'Avoid unnecessary absences.';
      else if (pct >= 0.70) insight = 'Attend all upcoming lectures.';
      else insight = 'High risk. Prioritize every class.';
    } else {
  ...
  static int _calculateClassesUntil(int present, int total, double targetPct, bool attending) {
    if (total == 0) return 0;
  ...
  static OverallHealth calculateOverallHealth(List<AttendanceRecord> records) {
    if (records.isEmpty) {
      return OverallHealth(score: 100, grade: 'A', trend: AttendanceTrend.stable, safeClassesRemaining: 0);
    }
    ...
    for (final r in records) {
      ...
      if (r.total > 0) {
        final safeForSubject = _calculateClassesUntil(r.present, r.total, thresholdWarning, false);
    ...
    final criticalSubjects = records.where((r) => r.total > 0 && r.percentage < thresholdCritical).length;
    score -= (criticalSubjects * 5); // Subtract 5 points for every critical subject
  ...
  static List<TodayRecommendation> generateTodayRecommendations(List<TimetableEntry> entries, List<AttendanceRecord> records) {
    ...
    for (final entry in uniqueEntries.values) {
      final record = records.where((r) => r.subjectCode == entry.subject && r.component == entry.component).firstOrNull;
      if (record == null || record.total == 0) {
        recommendations.add(TodayRecommendation(
      ...
      final pct = record.percentage;
      final safeMargin80 = _calculateClassesUntil(record.present, record.total, thresholdWarning, false);
      final safeMargin75 = _calculateClassesUntil(record.present, record.total, thresholdCritical, false);
      ...
      if (pct < thresholdCritical) {
        ...
        if (safeMargin75 <= 1) {
          ...
          reason = 'Attendance is in the warning zone. Missing classes increases risk rapidly.';
        }
        priority = 2;
      } else if (pct < thresholdSafe) {
        level = RecommendationLevel.recommended;
        reason = 'Attendance is decent but should be maintained to avoid entering the warning zone.';
        priority = 3;
      } else if (pct >= 0.95) {
        level = RecommendationLevel.comfortableMargin;
        reason = 'You have excellent attendance and can safely miss if necessary.';
        priority = 5;
      } else {
  ...
  static List<RecoveryPlan> generateRecoveryPlans(List<AttendanceRecord> records) {
    final plans = <RecoveryPlan>[];
    for (final r in records) {
      if (r.total > 0 && r.percentage < thresholdCritical) {
        final needed = _calculateClassesUntil(r.present, r.total, thresholdCritical, true);
        plans.add(RecoveryPlan(
          subjectCode: r.subjectCode,
          component: r.component,
          currentPercentage: r.percentage,
          targetPercentage: thresholdCritical,
          classesToAttend: needed,
          estimatedDays: needed * 3, // rough estimate, assuming 2-3 classes per week
        ));
      }
    }
    return plans;
  ...
  static double simulateScenario(AttendanceRecord record, {required int attendNext, required int missNext}) {
    if (record.total == 0) {
