...
class AttendanceParserService {
  ...
  static Future<void> parsePDFAndUpload(
    List<int> bytes,
    String division,
  ) async {
    ...
    try {
      ...
      final rowRegex = RegExp(
        r"^\s*(\d+)\s+(.+?)\s+([A-Z][a-z]{2} \d{1,2}, \d{4})\s+(\d{1,2}:\d{2}:\d{2} [AP]M)\s+(\d{1,2}:\d{2}:\d{2} [AP]M)\s+(P|A|E|L|NU)\s*$",
      );
      final courseRegex = RegExp(
        r"^(.*?)(?:[A-Z]\d\s*(?:CE)?\s*(?:Sem)?\s*(?:III|II|IV|V|VI|VII|VIII|I)?\s*(?:C\d)?)\s*$",
      );
      ...
      final lines = rawLines
          ...
          .toList();
      debugPrint(
        'PDF PARSER: Extracted ${rawLines.length} raw lines, ${lines.length} non-empty lines from PDF',
      );
      ...
      final uniqueCoursesMap =
          <
            String,
            Map<String, String>
          >{}; // Maps shortCode to {subjectCode, component}
      ...
      for (int i = 1; i < lines.length - 3; i++) {
        ...
        if (RegExp(r'^[A-Z][a-z]{2}\s\d{1,2},\s\d{4}$').hasMatch(line)) {
          ...
          if (courseRaw.toUpperCase().contains('P4') ||
              courseRaw.toUpperCase().contains('PRACTICAL') ||
              courseRaw.toUpperCase().contains('LAB')) {
            component = 'Lab';
          } else if (courseRaw.toUpperCase().contains('U4') ||
              courseRaw.toUpperCase().contains('TUTORIAL')) {
            component = 'Tutorial';
          }
          ...
          if (!aliasFound) {
            debugPrint(
              "WARNING: No alias found for '$course'. UI lookup might fail!",
            );
          }

          uniqueCoursesMap[subjectCode] = {
          ...
          if (statusRaw == 'P')
            ...
            statusStr = 'absent';
          else if (statusRaw == 'NU')
            statusStr = 'not updated';
          ...
          final deduplicationKey =
              '${division}_$year-$month-${day}_${startMins}_${subjectCode}_$component';
          final docRef = db
              .collection('users')
              .doc(uid)
              .collection('attendance_logs')
              .doc(deduplicationKey);
      ...
      final uniqueSubjects = uniqueCoursesMap.values
          .map((e) => e['subjectCode']!)
          .toSet();
      ...
      for (final subjectCode in uniqueSubjects) {
        ...
        if (subjectCode == 'SnS' || subjectCode == 'PnS') {
          final snapshot = await db
              ...
              .where(
                'status',
                whereIn: ['present', 'absent'],
              ) // Ignores component, grabs all
              .get();
          ...
          final recordId = '${division}_${subjectCode}_Theory'.replaceAll(
            RegExp(r'\s+'),
            '_',
          );
          final summaryRef = db
          ...
          summaryBatch.set(summaryRef, {
            ...
            'subjectCode': subjectCode,
            'component':
                'Theory', // Force 'Theory' so the UI doesn't spawn an extra Lab card
            'present': present,
            'absent': absent,
            'cancelled': 0,
          ...
          for (final component in componentsForSubject) {
            ...
            final recordId = '${division}_${subjectCode}_$component'.replaceAll(
              RegExp(r'\s+'),
              '_',
            );
            final summaryRef = db
      ...
      if (e.code == 'permission-denied') {
        throw Exception(
          "Database access denied. Please re-login or check permissions.",
        );
      }
      rethrow;
    } catch (e) {
