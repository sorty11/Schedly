...
class CourseConfigurationService {
  ...
  static Future<List<CourseComponent>> getMetadata(
    String division, {
              ...
              .map(
                (e) => CourseComponent(
                  componentId:
                      e['componentId'] ??
                      e['id'] ??
                      '', // Fallback to 'id' for cache transition
                  componentType:
                      e['componentType'] ??
                      ((e['isLab'] == true) ? 'Lab' : 'Theory'),
  ...
  static Future<List<Course>> getCourses(
    ...
    return grouped.entries.map((e) {
      ...
      final courseCode = comps
          .firstWhere((c) => c.courseCode.isNotEmpty, orElse: () => comps.first)
          .courseCode;
  ...
  static Stream<List<CourseComponent>> streamMetadata(String division) {
    return _db
        ...
        .map(
          (snap) =>
              snap.docs.map((d) => CourseComponent.fromFirestore(d)).toList(),
        );
  }
