...
class _ConductAdjustmentSheetState extends State<ConductAdjustmentSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final batchData = widget.subject.batches.firstWhere((b) => b.batch == _selectedBatch);
    ...
    return Padding(
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            Text('Adjust Conducted Hours', style: GoogleFonts.outfit(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: AppSpacing.sm),
            Text('Manually add or subtract hours. This will directly affect the "Adjusted" stat without altering historical logs.', 
            ...
            if (widget.subject.batches.length > 1) ...[
              DropdownButtonFormField<String>(
                ...
                items: widget.subject.batches.map((b) => DropdownMenuItem(
                  value: b.batch,
                  child: Text(b.batch),
                )).toList(),
            ...
            Text('Adjustment History', style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: AppSpacing.sm),
            ...
            if (widget.subject.batches.isNotEmpty)
              StreamBuilder<List<ConductAdjustment>>(
                stream: AnalyticsService.streamAdjustmentsForSubject(
                  division: AppSettings.sectionId ?? AppSettings.division ?? '',
                  subject: widget.subject.batches.firstWhere((b) => b.batch == _selectedBatch).subject,
                  component: widget.subject.batches.firstWhere((b) => b.batch == _selectedBatch).component,
                  batch: _selectedBatch,
                ),
                builder: (context, snapshot) {
                  ...
                  if (docs.isEmpty) {
                    return Text('No adjustments recorded yet.', style: TextStyle(color: sem.onSurfaceMuted, fontSize: 13));
                  }
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, i) {
                      ...
                      return ListTile(
                        ...
                        title: Text('${adj.adjustmentHours > 0 ? '+' : ''}${adj.adjustmentHours} hours', style: const TextStyle(fontWeight: FontWeight.w600)),
                        subtitle: Text('${adj.reason.isNotEmpty ? adj.reason : 'No reason'} • ${adj.createdBy}', style: TextStyle(fontSize: 12, color: sem.onSurfaceMuted)),
                      );
                    },
                  );
                },
...
void showConductAdjustmentSheet(BuildContext context, SubjectAnalytics subject) {
