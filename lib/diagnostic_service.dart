...
class DiagnosticService {
  static Future<String> runDiagnostics() async {
    ...
    try {
      ...
      log(
        '  - prefs.getString("selected_division"): ${prefs.getString("selected_division")}',
      );

      final divisionToQuery =
          AppSettings.sectionId ??
          AppSettings.division ??
          prefs.getString('selected_division') ??
          'Unknown';
      log(
        '  - Value that WOULD be passed to Rosters/Requests: $divisionToQuery',
      );
      ...
      try {
        final usersSnap = await FirebaseFirestore.instance
            .collection('users')
            .where('division', isEqualTo: divisionToQuery)
            .get();
        log(
          '    Found ${usersSnap.docs.length} users with division == $divisionToQuery',
        );
        if (usersSnap.docs.isNotEmpty) {
          log('    Sample User Data: ${usersSnap.docs.first.data()}');
        }

        final usersSnapSection = await FirebaseFirestore.instance
            .collection('users')
            .where('sectionId', isEqualTo: divisionToQuery)
            .get();
        log(
          '    Found ${usersSnapSection.docs.length} users with sectionId == $divisionToQuery',
        );
      } catch (e) {
      ...
      try {
        final facSnap = await FirebaseFirestore.instance
            ...
            .where('assignedDivisions', arrayContains: divisionToQuery)
            .get();
        log(
          '    Found ${facSnap.docs.length} faculty profiles with arrayContains: $divisionToQuery',
        );
        ...
        final anyFac = await FirebaseFirestore.instance
            .collection('faculty_profiles')
            .limit(1)
            .get();
        if (anyFac.docs.isNotEmpty) {
          log(
            '    Random Faculty Profile assignedDivisions: ${anyFac.docs.first.data()['assignedDivisions']}',
          );
        }
      } catch (e) {
      ...
      try {
        final membSnap = await FirebaseFirestore.instance
