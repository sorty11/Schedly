...
class _SrConductDashboardState extends State<SrConductDashboard> {
  ...
  void initState() {
    super.initState();
    _pendingLogsStream = AnalyticsService.streamPendingLogs(
      widget.division,
      widget.subject,
      null,
      null,
        ...
        return SafeArea(
          child: Padding(
            ...
            child: FutureBuilder<List<SrIdentity>>(
              future: TimetableManager.getUniqueSrIdentities(
                division: widget.division,
              ),
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Padding(
                              ...
                              child: AnimatedButton(
                                onPressed: () async {
                                  final reqDur =
                                      await TimetableManager.getSubjectRequiredDuration(
                                        division: widget.division,
                                        subject: subj.subject,
                                      );

                                  if (reqDur >= 120 &&
                                      log.durationMinutes < 120) {
                                    if (!context.mounted) return;
                                    AppDialogs.showError(
                                      context: context,
                                      title: 'Invalid Replacement',
                                      message:
                                          'This replacement requires a 2-hour continuous slot, but the selected lecture occupies only a ${log.durationMinutes}-minute period.',
                                    );
                                    return;
                                  }
                                  ...
                                  AppDialogs.showSnackBar(
                                    context: context,
                                    message:
                                        'Marked as Rescheduled to ${subj.subject} ${subj.component}',
                                  );
                                },
                                ...
                                child: Text(
                                  '${subj.subject} ${subj.component} (${batchNames[subj.batch] ?? subj.batch})',
                                  style: const TextStyle(
                                    fontWeight: FontWeight.bold,
                                    fontSize: 16,
                                  ),
                ...
                itemBuilder: (context, index) {
                  ...
                  return Column(
                    ...
                    children: [
                      ...
                      ...dateLogs.asMap().entries.map(
                        (e) => _buildLogCard(
                          e.value,
                          batchNames,
                          isFirst: index == 0 && e.key == 0,
                        ),
                      ),
  ...
  Widget _buildLogCard(
    ...
    final isWarning =
        parsedDate != null && DateTime.now().difference(parsedDate).inDays > 1;
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: isWarning
                    ? sem.error.withValues(alpha: 0.1)
                    : sem.pending.withValues(alpha: 0.1),
                    ...
                    children: [
                      ...
                      Text(
                        TimetableManager.formatTime(
                          log.originalSlot.startTime,
                          log.originalSlot.endTime,
                        ),
                        style: TextStyle(
                          fontSize: 13,
                          color: sem.onSurfaceMuted,
            ...
            Icon(Icons.chevron_right_rounded, color: sem.onSurfaceFaint),
          ],
        ),
      ),
    );
    ...
    if (isFirst) {
      return TutorialTarget(id: 'verify_lecture_btn', child: card);
    }
    return card;
  }
}
