...
import { verifyCRForSection } from './memberships';

export const removeFaculty = functions.https.onCall({ secrets: [] }, async (request) => {
  ...
  if (!targetUid || !sectionId) throw new functions.https.HttpsError('invalid-argument', 'Missing required fields.');

  // 1. Validate CR owns the section
  const isCR = await verifyCRForSection(auth.uid, sectionId);
  if (!isCR) throw new functions.https.HttpsError('permission-denied', 'Only active CRs can remove faculty from this section.');
  ...
  const assignedDivisions: string[] = facultyData.assignedDivisions || [];
  ...
  const subjectIds: string[] = subjectsMap[sectionId] || [];
