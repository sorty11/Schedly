...
void main() {
  test('Fetch Mismatch Proof', () async {
    ...
    for (final doc in oRes.docs) {
      ...
      if (role == 'faculty' || type == 'faculty_reminder' || type == 'lecture_added' || type == 'add' || type == 'edit') {
        print('4. Outbox (${doc.id}) -> division: ${data['division']}, role: ${data['role']}, uid: ${data['uid']}, title: ${data['title']}, processed: ${data['processed']}');
      }
    }
  });
}
