  ...
  test('Extract PDF', () async {
    ...
    for (int p = 0; p < 1; p++) {
      ...
      for (final word in words) {
        ...
        if ((t == 'sr' || t == 'no.') && cx < 100)
          srNoX = cx;
        else if (t == 'course' && cx > 100 && cx < 300)
          courseNameX = cx;
        else if (t == 'date' && cx > 250 && cx < 400)
          dateX = cx;
        else if (t == 'start' && cx > 350 && cx < 500)
          startTimeX = cx;
        else if (t == 'end' && cx > 450 && cx < 600)
          endTimeX = cx;
        else if ((t == 'attenda' || t == 'nce' || t == 'attendance') &&
            cx > 500)
          attendanceX = cx;
      }
      ...
      print(
        'srNoX: $srNoX, courseNameX: $courseNameX, dateX: $dateX, startTimeX: $startTimeX, endTimeX: $endTimeX, attendanceX: $attendanceX',
      );
      ...
      final colEnd = [
        (startTimeX + endTimeX) / 2,
        (endTimeX + attendanceX) / 2,
      ];
      ...
      for (final y in rowCenters) {
        final rowWords = words
            .where((w) => (w.bounds.center.dy - y).abs() <= 6)
            .toList();
        rowWords.sort(
        ...
        for (final w in rowWords) {
          ...
          if (cx >= colSrNo[0] && cx < colSrNo[1])
            srNo += '$t ';
          else if (cx >= colCourse[0] && cx < colCourse[1])
            course += '$t ';
          else if (cx >= colDate[0] && cx < colDate[1])
            dateStr += '$t ';
          else if (cx >= colStart[0] && cx < colStart[1])
            start += '$t ';
          else if (cx >= colEnd[0] && cx < colEnd[1])
            end += '$t ';
          else if (cx >= colStatus[0] && cx < colStatus[1])
            status += '$t ';
        }

        print(
          'DATE: [$dateStr] COURSE: [$course] START: [$start] END: [$end] STATUS: [$status]',
        );
      }
    }
  });
