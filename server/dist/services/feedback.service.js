    ...
    static async dispatchFeedbackEmail(reportId, providedData) {
        ...
        if (process.env.RESEND_API_KEY) {
            try {
                ...
                logger_1.logger.info('Feedback email delivered successfully via Resend HTTPS API', {
                    reportId,
                    emailId: resJson.id,
                });
                await docRef.update({
                ...
                logger_1.logger.error('Failed to send feedback email via Resend API', { reportId, error: err.message });
                await docRef.update({
                    emailStatus: 'failed',
